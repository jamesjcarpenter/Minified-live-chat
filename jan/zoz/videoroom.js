			...
			janus = new Janus(
				{
					...
					//				$('#username').focus();
                                    ...
                                    sfutest.send({"message": assumeRoom});
					...
					var ju = {"request" : "listforwarders",  "room" : myroom }
								...
								onlocalstream: function(stream) {
									...
									if($('#myvideo').length === 0) {
										...
										// Add a 'mute' button
										$('#videolocal').append('<button class="ui small green compact button" id="mute" style="position: absolute; bottom: 10%; left: 50%; margin:15px;"><i class="volume up icon">PUSH TO TALK</i></button>');
                                        $('#mute').click(toggleMute);
                                        $('#mute').click();
										// Add an 'unpublish' button
										$('#videolocal').append('<button class="ui small red compact button" id="unpublish" style="position: absolute; bottom: 10%; left: 50%; margin: 15px;"><i class="times circle icon"></i></button>');
	...
	janus.attach(
		{
			...
			//			$('#curres'+remoteFeed.rfindex).removeClass('hide').text(width+'x'+height).show();
				...
				//		$('#curbitrate'+remoteFeed.rfindex).text(bitrate);
						...
						var height = $("#remotevideo"+remoteFeed.rfindex).get(0).videoHeight;
						if(width > 0 && height > 0)
						$('#curres'+remoteFeed.rfindex).removeClass('hide').text(width+'x'+height).hide();
					}, 1000);
				}
			},
			oncleanup: function() {
